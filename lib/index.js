 ...
 * @errors  There should be none as globby will just return an empty array if no matches are found.
 */
 ...
 * @returns {Object}        Expected: A more expanded config that includes the section.files source code, Error: an error object
 * @errors  If fsp.readFile returns a rejected response, we will bubble that error up and reject the promise. (it will silently fail, !!!we will need a catch for that!!!)
 */
 ...
 * @param  {Object} config Expanded config
 * @returns {Object}        Expected: A more expanded config, Error: an error object
 * @errors  If parseComment returns an error instead of the data, we should bubble that error up and reject the promise.
 */
function parseSections( config ) {
    ...
    if ( isError ) return Promise.reject( error );
        ...
        // console.log(util.inspect( config, { depth: 9, colors: true } ) );
